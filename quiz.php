...
add_filter( 'wpcf7_validate_quiz', 'dnp_cf7_quiz_validation_filter', 10, 2 );

function dnp_cf7_quiz_validation_filter( $result, $tag ) {
	$tag = new WPCF7_Shortcode( $tag );
...
add_filter( 'wpcf7_ajax_onload', 'dnp_cf7_quiz_ajax_refill' );
add_filter( 'wpcf7_ajax_json_echo', 'dnp_cf7_quiz_ajax_refill' );

function dnp_cf7_quiz_ajax_refill( $items ) {
	if ( ! is_array( $items ) )
		return $items;
...
add_filter( 'wpcf7_messages', 'dnp_cf7_quiz_messages' );

function dnp_cf7_quiz_messages( $messages ) {
	return array_merge( $messages, array( 'quiz_answer_not_correct' => array(
		'description' => __( "Sender doesn't enter the correct answer to the quiz", 'cf7e' ),
		'default' => __( 'Your answer is not correct.', 'cf7e' )
	) ) );
}
...
add_action( 'admin_init', 'dnp_cf7_add_tag_generator_quiz', 40 );

function dnp_cf7_add_tag_generator_quiz() {
	if ( ! function_exists( 'wpcf7_add_tag_generator' ) )
		return;

	wpcf7_add_tag_generator( 'quiz', __( 'Quiz', 'cf7e' ),
		'wpcf7-tg-pane-quiz', 'dnp_cf7_tg_pane_quiz' );
}

function dnp_cf7_tg_pane_quiz( $contact_form ) {
?>
<div id="wpcf7-tg-pane-quiz" class="hidden">
<form action="">
<table>
<tr><td><?php echo esc_html( __( 'Name', 'cf7e' ) ); ?><br /><input type="text" name="name" class="tg-name oneline" /></td><td></td></tr>
</table>
...
<tr>
<td><code>size</code> (<?php echo esc_html( __( 'optional', 'cf7e' ) ); ?>)<br />
<input type="number" name="size" class="numeric oneline option" min="1" /></td>

<td><code>maxlength</code> (<?php echo esc_html( __( 'optional', 'cf7e' ) ); ?>)<br />
<input type="number" name="maxlength" class="numeric oneline option" min="1" /></td>
</tr>

<tr>
<td><?php echo esc_html( __( 'Quizzes', 'cf7e' ) ); ?><br />
<textarea name="values"></textarea><br />
<span style="font-size: smaller"><?php echo esc_html( __( "* quiz|answer (e.g. 1+1=?|2)", 'cf7e' ) ); ?></span>
</td>
</tr>
</table>

<div class="tg-tag"><?php echo esc_html( __( "Copy this code and paste it into the form left.", 'cf7e' ) ); ?><br /><input type="text" name="quiz" class="tag wp-ui-text-highlight code" readonly="readonly" onfocus="this.select()" /></div>
</form>
</div>
<?php
}
